...
HOTEL_SEARCH_API_URL="http://api.tbotechnology.in/TBOHolidays_HotelAPI/Search"
HOTEL_DETAILS_API_URL ="http://api.tbotechnology.in/TBOHolidays_HotelAPI/HotelDetails"
...
def chat_with_llm(user_input):
    ...
    prompt = (
        ...
        "If the user seeks suggestions or seems unsure, provide helpful recommendations. If a destination image's details are provided, suggest locations that closely match the description. "
...
def get_results(payload):
    try:
        response = requests.post(HOTEL_SEARCH_API_URL, json=payload, auth=HTTPBasicAuth(username, password))
...
def get_hotel_info(payload):
    try:
        response = requests.post(HOTEL_DETAILS_API_URL , json=payload, auth=HTTPBasicAuth(username, password))
        if response.status_code == 200:
            response_data = response.json()  # This will convert the JSON response to a Python dictionary
            return response_data
...
def main():
    ...
    if len(parameters["Attractions"]):  
        ...
        print("here5")
        hotel_results=get_hotel_info({"Hotelcodes": filtered_hotelcodes, "Language": "EN"}) 
        print("hotel_results5", hotel_results)
