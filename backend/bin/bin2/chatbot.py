...
# import fetchCountryCode, fetchCityCode, fetchHotelCodes, additional_filters
# import requests
...
# GROQ_API_KEY = "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"
...
#         "'Budget' parameter: Ask user their budget for each hotel room(must be same for all rooms)"
#         "'Amenities' parameter: Ask and extract the amenities the user wants and store all the amenities in an array. "
...
#                     "'Filters: A dictionary containing 'Refundable':(true/false), 'NoOfRooms':(a non-negative integer), 'MealType':('All', 'WithMeal', or 'RoomOnly')', 'Budget: positive number', 'Amenities: [amenity1, amenity2,...]'\n"
#                     f"Conversation:\n{session_history}"
#                 )
...
#     searchresults=get_results(parameters)
#     filtered_hotelcodes=additional_filters.budget_filter(searchresults,travel_preferences["Budget"])
#     hotelresults=get_hotel_info({"Hotelcodes": filtered_hotelcodes, "Language": "EN"})
#     filtered_hotelcodes=additional_filters.amenities_filter(hotelresults,parameters["Amenities"])
